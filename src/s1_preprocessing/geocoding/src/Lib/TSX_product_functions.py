# ---- This is <S1_product_functions.py> ----

"""
Collection of useful simple functions for working with TSX data.
"""
...
def get_xml_path(product_folder_path):
    basename = product_folder_path.name
    xml_name = basename + '.xml'
    xml_path = pathlib.Path.joinpath(product_folder_path, xml_name)
    return xml_path
...
def get_S1_datestring(f_base):
  ...
  try:
    ...
    logger.warning(
      ...
      'recommend using S1 naming conventions'
    )
...
def get_S1_product_info(f_base):
  ...
  if product_pol == '1SDH':
    ...
    logger.warning(
      f'Found unknown polarization string in S1 basename: {product_pol}'
    )
    product_pols = []
...
# ---- End of <S1_product_functions.py> ----
