...
class S1IcebergArea:
    def __init__(self, dir_tmp) -> None:
    ...
    def run_model(self, file_s1=None):
        """
        Runs a gamma CFAR and the BackscatterRL CatBoost (CB) area model.
        :param: file_s1 str file path of the Sentinel-1 EW GRDM data as output of prepare_s1().
        """
        ...
        for i, pol in enumerate(["hh", "hv"]):
            ...
            icebergs[pol] = self.icebergs.copy()
        return icebergs  # contains CFAR area and predicted area, one GeoDataFrame per channel
    ...
    def _extract_backscatter_stats(self, clutter, contrast):
        ...
        for i, (key, data) in enumerate(zip(["clutter", "contrast"], [clutter, contrast])):
            for pol_idx, pol in enumerate(["hh", "hv"]):
                stats = zonal_stats(self.icebergs, self.prep.decibels_to_linear(data[pol_idx]), affine=self.meta_s1["transform"], nodata=np.nan)  # calc stats in linear intensities
...
if __name__ == "__main__":
    iceberg_area = S1IcebergArea(dir_tmp)
    #iceberg_area.prepare_s1("")
    file_s1 = "/media/henrik/DATA/FS_Cruise_2024/data/s1/S1A_EW_GRDM_1SDH_20240812T075359_20240812T075503_055177_06B983_C591_3996_40_rm_thermal_noise_hvFalse_rm_texture_noise_hvFalse.tif"
    icebergs_hh = iceberg_area.run_model()
